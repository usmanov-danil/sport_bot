...
class DialogCalendar:
    ...
    async def start_calendar(self, year: int = datetime.now().year) -> InlineKeyboardMarkup:
        ...
        inline_kb.insert(
            InlineKeyboardButton(
                KEYBOARD['prev'], callback_data=calendar_callback.new("PREV-YEARS", year, -1, -1)
            )
        )
        inline_kb.insert(
            InlineKeyboardButton(
                KEYBOARD['next'], callback_data=calendar_callback.new("NEXT-YEARS", year, -1, -1)
            )
        )
    ...
    async def _get_month_kb(self, year: int):
        ...
        inline_kb.insert(
            InlineKeyboardButton(year, callback_data=calendar_callback.new("START", year, -1, -1))
        )
        inline_kb.insert(InlineKeyboardButton(" ", callback_data=ignore_callback))
